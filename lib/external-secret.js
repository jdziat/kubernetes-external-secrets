...
async function startWatcher ({
  ...
  const watchPath = namespace
    ? `/apis/${group}/v1/namespaces/${namespace}/${plural}`
    : `/apis/${group}/v1/${plural}`
  ...
  try {
    while (true) {
      ...
      const restartTimeout = () => {
        ...
        timeout = setTimeout(() => {
          logger.info(`No watch event for ${watchTimeout} ms, restarting watcher for ${loggedNamespaceName}`)
          if (aborter) {
            aborter.abort()
          }
        }, watchTimeout)
      ...
      const aborter = await kubeClient.watch.watch(
        watchPath,
        {},
        (type, apiObj) => {
          eventQueue.put({ type, object: apiObj })
          restartTimeout()
        },
        (err) => {
          if (err) {
            logger.warn(err, 'Got error on watch for namespace %s', loggedNamespaceName)
