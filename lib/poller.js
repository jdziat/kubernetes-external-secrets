...
class Poller {
  ...
  async _upsertKubernetesSecret () {
    const ns = await this._kubeClient.core.readNamespace({ name: this._namespace })
    const verdict = this._isPermitted(ns, this._spec)
    ...
    try {
      ...
      existingSecret = await this._kubeClient.core.readNamespacedSecret({ name: this._name, namespace: this._namespace })
    } catch (err) {
    ...
    if (existingSecret && this._equalSecretData(existingSecret, secretManifest)) {
      ...
      return await this._kubeClient.core.createNamespacedSecret({ namespace: this._namespace, body: secretManifest })
    } else {
      this._logger.info(`updating secret ${this._namespace}/${this._name}`)
      return this._kubeClient.core.replaceNamespacedSecret({ name: this._name, namespace: this._namespace, body: secretManifest })
    }
  }
  ...
  async _updateStatus (status) {
    ...
    try {
      ...
      await this._kubeClient.customObjects.replaceNamespacedCustomObjectStatus({
        group: this._customResourceManifest.spec.group,
        version: 'v1',
        namespace: this._namespace,
        plural: this._customResourceManifest.spec.names.plural,
        name: this._name,
        body: {
  ...
  async _scheduleNextPoll () {
    try {
      const statusObject = await this._kubeClient.customObjects.getNamespacedCustomObjectStatus({
        group: this._customResourceManifest.spec.group,
        version: 'v1',
        namespace: this._namespace,
        plural: this._customResourceManifest.spec.names.plural,
      ...
      const { lastSync = null, observedGeneration = 0 } = (statusObject && statusObject.status) || {}
